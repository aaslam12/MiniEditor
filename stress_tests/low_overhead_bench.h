...
// ═══════════════════════════════════════════════════════════════════════════════
// Low-Overhead Benchmark Utilities (copied from Palloc)
//
// Optimizations:
...
inline double estimate_cycles_per_second(int sample_ms = 200)
{
    ...
    if (elapsed.count() <= 0.0)
        return 1.0;
    return static_cast<double>(end_cycles - start_cycles) / elapsed.count();
}

inline double cycles_to_seconds(double cycles, double cycles_per_second)
{
...
inline std::string format_seconds(double seconds)
{
    char buffer[64];
    if (seconds < 1e-6)
        std::snprintf(buffer, sizeof(buffer), "%.2f ns", seconds * 1e9);
